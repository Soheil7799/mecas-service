...
class FilterConfig:
    def __init__(self):
        ...
        self.gain_compressor_threshold = -1.0
        self.limiter_threshold = 0.0

        self.voice_enhance_apply = False
        self.preemphasis_alpha = 3.0
        self.high_pass_order = 2.0
        ...
        self.phone_side_gain = 0.0
        self.phone_filter_order = 1.0

        self.car_like_apply = False
        self.car_side_gain = 3.0
        self.car_filter_order = 1.0
...
def toggle_button(label, key):
    """Create a toggleable button that acts like a checkbox"""
    # Initialize the state if it doesn't exist
    if key not in st.session_state:
        st.session_state[key] = False

    # Create button with color based on state
    button_style = "primary" if st.session_state[key] else "secondary"
    prefix = "✓ " if st.session_state[key] else "○ "

    if st.button(f"{prefix}{label}", key=f"btn_{key}", type=button_style):
        # Toggle the state when clicked
        st.session_state[key] = not st.session_state[key]
        # Return true to indicate button was just clicked (for immediate UI update)
        return True
    return False
        ...
        with st.container(border=True):
            ...
            with st.container(border=True):
                ...
                with col1:
                    ...
                    if toggle_button("Gain Compressor", "gain_checkbox"):
                        st.session_state.filter_config.gain_apply = st.session_state.gain_checkbox
                    ...
                    if toggle_button("Voice Enhancement", "voice_enhance_checkbox"):
                        st.session_state.filter_config.voice_enhance_apply = st.session_state.voice_enhance_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.voice_enhance_apply = st.session_state.voice_enhance_checkbox
                ...
                with col2:
                    ...
                    if toggle_button("Denoise + Delay", "denoise_delay_checkbox"):
                        st.session_state.filter_config.denoise_delay_apply = st.session_state.denoise_delay_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.denoise_delay_apply = st.session_state.denoise_delay_checkbox
                    ...
                    # Phone-like Filter
                    if toggle_button("Phone-like Filter", "phone_like_checkbox"):
                        st.session_state.filter_config.phone_like_apply = st.session_state.phone_like_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.phone_like_apply = st.session_state.phone_like_checkbox
                    ...
                    # Car-like Filter
                    if toggle_button("Car-like Filter", "car_like_checkbox"):
                        st.session_state.filter_config.car_like_apply = st.session_state.car_like_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.car_like_apply = st.session_state.car_like_checkbox
            ...
            with st.container(border=True):
                ...
                with col3:
                    # Grayscale Filter
                    if toggle_button("Grayscale", "gray_scale_checkbox"):
                        st.session_state.filter_config.gray_scale_apply = st.session_state.gray_scale_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.gray_scale_apply = st.session_state.gray_scale_checkbox

                    # Color Invert Filter
                    if toggle_button("Color Invert", "invert_checkbox"):
                        st.session_state.filter_config.invert_apply = st.session_state.invert_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.invert_apply = st.session_state.invert_checkbox

                    # Frame Interpolation Filter
                    if toggle_button("Frame Interpolation", "frame_interpolate_checkbox"):
                        st.session_state.filter_config.frame_interpolate_apply = st.session_state.frame_interpolate_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.frame_interpolate_apply = st.session_state.frame_interpolate_checkbox
                ...
                with col4:
                    # Upscale Filter
                    if toggle_button("Upscale", "upscale_checkbox"):
                        st.session_state.filter_config.upscale_apply = st.session_state.upscale_checkbox
                        st.rerun()
                    else:
                        st.session_state.filter_config.upscale_apply = st.session_state.upscale_checkbox
            ...
            # Apply Filters Button - made more prominent
        ...
        if st.session_state.video_processed:
            try:
                ...
                if stream_response.status_code == 200:
                    ...
                    st.video(stream_response.content)
                else:
